    ...
    config = function(_, opts)
      ...
      local function resolve_rust_analyzer()
        ...
        local cargo_bin = cargo_home
          .. (vim.g.is_windows and "/bin/rust-analyzer.exe" or "/bin/rust-analyzer")
      ...
      local servers = {
        ...
        jdtls = {
          ...
          cmd = function(dispatchers, config)
            local project = config.root_dir and vim.fs.basename(config.root_dir) or "default"
            local base = vim.fn.stdpath("cache") .. "/jdtls/" .. project
            local exe = vim.fn.stdpath("data") .. "\\mason\\bin\\jdtls.cmd"
            local cmd = {
              exe,
              "-configuration",
              base .. "/config",
        ...
        tsgo = {
          cmd = {
            vim.fn.stdpath("data") .. "\\mason\\bin\\tsgo.cmd",
            "--lsp",
            "--stdio",
          },
          root_dir = function(bufnr, on_dir)
